...
const carsTableRowFn = record => ({ onClick: () => openCarDrawer(record) })
const driversTableRowFn = record => ({ onClick: () => openDriverDrawer(record) })
...
<!--                            />-->
        ...
        <template #rightExtra>
            <a-button v-if="currentMainTab === 'vehicles'" @click="() => openCarDrawer()">Добавить машину</a-button>
            <a-button v-if="currentMainTab === 'drivers'" @click="() => openDriverDrawer()">Добавить водителя</a-button>
        </template>
    </a-tabs>
